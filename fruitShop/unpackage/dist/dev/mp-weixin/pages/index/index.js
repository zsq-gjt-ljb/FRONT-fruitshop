...
const _sfc_main = {
  ...
  setup(__props, { expose: __expose }) {
    ...
    const loadCategories = async () => {
      try {
        ...
        if (result.code === 200) {
          ...
          common_vendor.index.__f__("log", "at pages/index/index.vue:322", "处理后的分类数据:", categories.value);
        } else {
          common_vendor.index.__f__("error", "at pages/index/index.vue:324", "获取分类失败:", result.message);
          categories.value = [];
        }
      } catch (error) {
        common_vendor.index.__f__("error", "at pages/index/index.vue:328", "加载分类出错:", error);
        categories.value = [];
      }
    };
    const loadHomeProducts = async () => {
      try {
        common_vendor.index.__f__("log", "at pages/index/index.vue:336", "开始加载首页推荐商品");
        const result = await utils_request.request({
          url: "https://bgnc.online/api/product/market/0",
          method: "GET"
        });
        common_vendor.index.__f__("log", "at pages/index/index.vue:342", "首页商品API返回结果:", result);
        if (result.code === 200) {
          common_vendor.index.__f__("log", "at pages/index/index.vue:344", "首页商品数据:", result.data);
          ...
          common_vendor.index.__f__("log", "at pages/index/index.vue:352", "处理后的首页商品数据:", homeProducts.value);
        } else {
          common_vendor.index.__f__("error", "at pages/index/index.vue:354", "获取首页商品失败:", result.message);
          homeProducts.value = [];
        }
      } catch (error) {
        common_vendor.index.__f__("error", "at pages/index/index.vue:358", "加载首页商品出错:", error);
        homeProducts.value = [];
      }
    };
    const initData = async () => {
      ...
      try {
        common_vendor.index.__f__("log", "at pages/index/index.vue:367", "开始初始化首页数据");
        await loadBanners();
        await loadCategories();
        await loadHomeProducts();
        common_vendor.index.__f__("log", "at pages/index/index.vue:373", "首页数据加载完成");
      } catch (error) {
        common_vendor.index.__f__("error", "at pages/index/index.vue:375", "初始化数据失败:", error);
    ...
    const goToDetail = (productId) => {
      if (!productId) {
        common_vendor.index.__f__("error", "at pages/index/index.vue:388", "无效的商品ID");
        return;
      }
      common_vendor.index.__f__("log", "at pages/index/index.vue:392", "首页跳转到商品详情，ID:", productId);
      const id = typeof productId === "object" ? productId.id : productId;
      common_vendor.index.navigateTo({
        url: `/pages/detail/detail?id=${id}`,
        success: (res) => {
          common_vendor.index.__f__("log", "at pages/index/index.vue:398", "跳转成功:", res);
        },
        fail: (err) => {
          common_vendor.index.__f__("error", "at pages/index/index.vue:401", "跳转失败:", err);
          common_vendor.index.navigateTo({
    ...
    const navigateToCategoryById = (categoryId, categoryName) => {
      common_vendor.index.__f__("log", "at pages/index/index.vue:414", "跳转到分类页面:", categoryId, categoryName);
    ...
    common_vendor.onMounted(() => {
      common_vendor.index.__f__("log", "at pages/index/index.vue:455", "首页组件已挂载");
      initData();
    });
    ...
    const addToCartWithSpec = async () => {
      ...
      try {
        ...
        common_vendor.index.__f__("log", "at pages/index/index.vue:564", "添加到购物车数据:", cartData);
        const result = await utils_request.request({
        ...
        common_vendor.index.__f__("error", "at pages/index/index.vue:589", "添加到购物车出错:", error);
        common_vendor.index.showToast({
          title: "网络错误，请重试",
          icon: "none"
        });
    ...
    const showQuickBuy = async (product) => {
      common_vendor.index.__f__("log", "at pages/index/index.vue:633", "显示快速购买弹窗:", product);
      ...
      try {
        ...
        if (result.code === 200 && result.data) {
          common_vendor.index.__f__("log", "at pages/index/index.vue:657", "获取到商品详情:", result.data);
        ...
        common_vendor.index.__f__("error", "at pages/index/index.vue:722", "获取商品详情失败:", error);
        productSpecs.value = [{ name: "默认规格", price: product.price, stock: 100 }];
        selectedSpec.value = "默认规格";
        quickBuyProduct.value.stock = 100;
      }
    ...
    return (_ctx, _cache) => {
      return common_vendor.e({
        ...
        f: common_vendor.f(categories.value.slice(0, 8), (category, index, i0) => {
          return {
